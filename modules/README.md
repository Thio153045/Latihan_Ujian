...
3. **Siswa login** (password awal wajib diganti saat login pertama), buka
   ...
   muncul, dan hanya yang BELUM pernah dikerjakan. Jawab tiap soal, tekan
   **Kumpulkan Jawaban** — jawaban tersimpan (belum dinilai), lalu otomatis
   lanjut ke paket berikutnya yang belum dikerjakan.
4. **Guru membuka halaman 🧮 Analisa Jawaban** untuk menilai jawaban siswa:
   - Filter berdasarkan nama siswa dan/atau paket soal, pilih submisi mana
     saja yang mau dianalisis (bisa banyak siswa sekaligus).
   - Klik **Analisa dengan AI** — AI menentukan jawaban benar + penjelasan
     singkat per soal (hanya potongan gambar per-soal yang dikirim ke API,
     bukan seluruh PDF, supaya hemat token), skor dihitung otomatis, dan
     hasil tersimpan sebagai nilai siswa.
   - Unduh **PDF** per siswa (untuk dipelajari ulang), atau **Excel rekap**
     sekaligus kalau menganalisis/melihat lebih dari satu siswa.
5. Siswa bisa buka halaman **📊 Riwayat Nilai** kapan saja untuk melihat
   status ("menunggu dinilai" atau skor final) dan mengunduh ulang PDF-nya
   setelah guru selesai menganalisis.
...
│   ├── 2_Kerjakan_Soal.py          # Siswa: kerjakan & kumpulkan jawaban (tanpa AI)
│   ├── 3_Kelola_Siswa.py           # Guru: buat akun siswa, atur akses paket soal
│   ├── 4_Riwayat_Nilai.py          # Siswa: lihat status/nilai & unduh ulang PDF
│   └── 5_Analisa_Jawaban.py        # Guru: analisa AI (per/banyak siswa), unduh PDF/Excel
├── modules/
│   ├── db.py                       # Koneksi MySQL (SQLAlchemy, cached) + migrasi skema
│   ├── auth_db.py                  # Login, sesi, buat akun guru/siswa, ganti password
│   ├── storage.py                  # Paket soal, akses, submit & analisa hasil ujian
...
│   ├── pdf_report.py               # Buat PDF hasil per siswa (ReportLab)
│   ├── excel_report.py             # Buat rekap Excel untuk banyak siswa (openpyxl)
